...
public class ObstacleActor extends DamageableActor {
	...
	public void damage(float angle) {
		...
		if(targetSizeType != 0) {
			...
			ObstacleActor asteroid0 = MainGameScreen.getInstance().addAsteroid();
			asteroid0.setSizeType(targetSizeType);
			asteroid0.setParams(angle0, speed0);
			asteroid0.setPosition(pos0X, pos0Y);
			
			ObstacleActor asteroid1 = MainGameScreen.getInstance().addAsteroid();
			...
			if(mNumAsteroidsKilled >= mNextAsteroidSize) {
				...
				ObstacleActor asteroid = MainGameScreen.getInstance().addAsteroid();
